...
\begin{itemize}
 ...
 \item execution - Manager adjusts container settings using an OpenVZ agent, located on computing node where container is deployed. As for OpenVZ hypervisor, container's properties are in fact User Bean Counter (UBC), exposed through a REST interface.
\end{itemize}
...
CPU is in legal range & \texttt{LESSER\_CPU} & Decrease CPU \\ \hline
CPU is lesser than allowed limit & \texttt{FITS\_CPU} & None \\ \hline
