...
During planning phase, procedure to enact desired state in resource is created. Depending on problem it may involve only one step on a complex workflow. Before implementing planning module, following issues should be taken into consideration:
\begin{asparaenum}
...
Execution module role is simple yet it plays a role that cannot be underestimated: carrying procedure scheduled beforehand during planning. To do so, it leverages effectors and API exposed by a container. In case of a self-learning system, change plan and its effects should be recorded to serve as a reference point in future.
