...
% Istniejące rozwiązania
	% OneFlow, OpenShift - D
	% Carina, CloudFoundry - R
% Propozycja rozwiązania
	% Adresowane problemy - R
	% Kontekst użycia - R
	% system adaptacyjny, oparty o model IBM (bo...) - R
	% overview (IBM) - D
	% nizsze warstwy - D
	% warstwa najwyzsza (orkiestracja) - R
% Implementacja
	% Overview - R
	% Cloud broker - R
	% Auto-scaling subsystem - D
% Ewaluacja
	% overview -R
	% autoscaling - R
	% autoscaling - multiple - R 
	% other - R
% Wnioski - R
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{assets/csap-layers}
    \caption{IBM. An architectural blueprint for autonomic computing}
  \end{center}
\end{figure}
\end{frame}
